...
$stmt = $pdo->query("
  SELECT c.*, u.email,
         IFNULL(e.total_empenhado, 0) AS total_empenhado
  FROM contratos c
...
while ($c = $stmt->fetch(PDO::FETCH_ASSOC)) {
  $saldo = $c['valor_total'] - $c['total_empenhado'];
  ...
  try {
    ...
    $mail->Body = "
      <p>O contrato nº <strong>{$c['numero']}</strong> está com saldo abaixo de 10%.</p>
      <p><strong>Processo:</strong> {$c['processo']}<br>
         <strong>Órgão:</strong> {$c['orgao']}<br>
         <strong>Saldo atual:</strong> R$ " . number_format($saldo, 2, ',', '.') . "</p>";
